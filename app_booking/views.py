...
# def booking_view(request):
#     """A foglalási oldal megjelenítése és a naptár adatok kiszolgálása"""
#     today = datetime.today()
#     year = request.GET.get("year", today.year)
#     month = request.GET.get("month", today.month)

#     # Konvertáljuk az év és hónap értékeket egész számokká
#     try:
#         year = int(year)
#         month = int(month)
#     except ValueError:
#         year, month = today.year, today.month  # Ha hiba van, térjünk vissza az aktuális hónapra

#     # A hónap első napja és utolsó napja
#     first_day, num_days = calendar.monthrange(year, month)
#     month_days = [datetime(year, month, day) for day in range(1, num_days + 1)]

#     # Lekérjük a nyitvatartási időket és a foglalt időpontokat
...
#         for day in week:
#             if day == 0:
#                 week_data.append({"date": None, "status": "empty"})
#                 continue

#             current_date = datetime(year, month, day)
#             day_of_week = current_date.weekday()  # 0 = hétfő, 6 = vasárnap
#             is_even_week = (current_date.isocalendar()[1] % 2) == 0  # Páros vagy páratlan hét

#             # Ellenőrizzük, hogy van-e nyitvatartás az adott napra
#             available_slots = []
#             for opening in opening_hours.filter(day_of_week=day_of_week, is_even_week=is_even_week):
#                 current_slot = opening.start_time
#                 while current_slot < opening.end_time:
#                     end_slot = (datetime.combine(current_date, current_slot) + timedelta(minutes=15)).time()

#                     # Foglaltság ellenőrzése
#                     is_taken = bookings.filter(date=current_date, start_time=current_slot).exists()
#                     if not is_taken:
#                         available_slots.append(current_slot.strftime("%H:%M"))

#                     current_slot = end_slot  # Lépjünk a következő negyedórás intervallumra

#             # Státusz beállítása (zöld: van elérhető időpont, piros: nincs)
#             day_status = "green" if len(available_slots) >= 2 else "red"
